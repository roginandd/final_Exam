...
const OrderList = ({ orders, setOrders, clerkName, setMessage, setStatus}) => {
  ...
  const [updatedOrder, setUpdatedOrder] = useState({ orderName: '', price: '', isDiscounted: false });
  const [isLoaded, setIsLoaded] = useState(true);

  const fetchOrders = async () => {
    try {
      const response = await axios.get('http://localhost:8082/kopeetearia-api/orders');
  ...
  const fetchTotal = async () => {
    try {
      const response = await axios.get('http://localhost:8082/kopeetearia-api/total-bill');
      setTotalRegular(response.data.data.regularBillTotal);
      setTotalDiscounted(response.data.data.discountedBillTotal);
    } catch (error) {
  ...
  const handleUpdate = async (id) => {
    try {
      const response = await axios.put(`http://localhost:8082/kopeetearia-api/update/${id}`, {
  ...
  const deleteOrder = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:8082/kopeetearia-api/delete/${id}`);
      if (response.data.success) {
        setMessage(response.data.message);
